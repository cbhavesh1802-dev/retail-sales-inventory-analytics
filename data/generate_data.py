"""
Generates sample retail/e-commerce sales and inventory data:
...
for d in range(DAYS):
    ...
    n_orders_today = int(np.random.poisson(150) * dow_factor * trend_factor * season_factor)
...
sales_df.to_csv(os.path.join(SCRIPT_DIR, "sales.csv"), index=False)
inventory_df.to_csv(os.path.join(SCRIPT_DIR, "inventory.csv"), index=False)

low_stock_count = (inventory_df["stock_status"] == "LOW").sum()
